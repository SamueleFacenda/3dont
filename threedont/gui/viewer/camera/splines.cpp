...
template <typename T> Spline<T>::Spline(const std::vector<T> &ts, const std::vector<T> &vs) : _ts(ts), _vs(vs) {
    checkAndInit();
}

template <typename T> Spline<T>::~Spline() {}

template <typename T> bool Spline<T>::checkTs(std::vector<T> &ts) {
...
template <typename T> void Spline<T>::checkAndInit() {
...
template <typename T> ConstantSpline<T>::~ConstantSpline() {}

template <typename T> T ConstantSpline<T>::eval(T t) const {
...
template <typename T> LinearSpline<T>::~LinearSpline() {}

template <typename T> T LinearSpline<T>::eval(T t) const {
    auto it = std::upper_bound(this->_ts.begin(), this->_ts.end(), t);
    int idx = it - this->_ts.begin();
    if (idx == 0)
        return this->_vs.front();
...
template <typename T> CubicSpline<T>::~CubicSpline() {}

template <typename T> T CubicSpline<T>::eval(T t) const {
    auto it = std::upper_bound(this->_ts.begin(), this->_ts.end(), t);
    int idx = it - this->_ts.begin();
    if (idx == 0)
        return this->_vs.front();
...
template <typename T> void CubicSpline<T>::calculateCoefficients() {
    int num_intervals = (int)this->_ts.size() - 1;
    if (num_intervals > 0) {
        SpMat A;
        Eigen::VectorXf b, x;
