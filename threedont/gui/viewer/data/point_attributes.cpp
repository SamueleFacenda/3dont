...
bool PointAttributes::set(const std::vector<float> &attr, quint64 attr_size, quint64 attr_dim) {
...
bool PointAttributes::set(const std::vector<char> &data, const Octree &octree) {
...
const std::vector<float> &PointAttributes::operator[](int i) const {
  return _attr[i];
}

float PointAttributes::operator()(int i, int j) const {
...
float PointAttributes::operator()(int k, int i, int j) const {
...
std::size_t PointAttributes::currentIndex() const {
  return _curr_idx;
}

quint64 PointAttributes::size(int i) const {
  return _attr_size[i];
}

quint64 PointAttributes::dim(int i) const {
  return _attr_dim[i];
}

std::size_t PointAttributes::numAttributes() const {
  return _attr.size();
}

void PointAttributes::setCurrentIndex(std::size_t i) {
  if (i < _attr.size() && i >= 0) _curr_idx = i;
}

bool PointAttributes::_unpack(const std::vector<char> &data, unsigned int expected_size) {
...
void PointAttributes::_reorder(std::size_t attr_idx, const Octree &octree) {
...
void PointAttributes::_compute_LOD(std::size_t attr_idx, const Octree &octree) {
  // make space for LOD averages
  std::size_t num_centroids = octree.getPointPos().size() / 3 - octree.getNumPoints();
  _attr[attr_idx].resize(_attr[attr_idx].size() + _attr_dim[attr_idx] * num_centroids);
...
void PointAttributes::_compute_rgba_LOD_helper(std::size_t attr_idx, const Octree::Node *node) {
...
void PointAttributes::_compute_LOD_helper(std::size_t attr_idx, const Octree::Node *node) {
  if (node == nullptr) return;
...
void PointAttributes::_accumulate_rgba(QVector3D &x, float &w, const float *v) {
  for (int i = 0; i < 3; i++) x[i] += v[i];
  w *= (1.0f - v[3]);
}

void PointAttributes::_xw_to_rgba(float *dst, const QVector3D &x, const float &w, unsigned int n) const {
  dst[0] = x[0] / n;
  dst[1] = x[1] / n;
  dst[2] = x[2] / n;
  dst[3] = 1.0f - w;
